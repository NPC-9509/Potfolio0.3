// AudioEngine.js — Upgraded audio system (from audio.js, Phase 13)
// Preserves all original procedural synthesis, adds section ambience, volume control, localStorage persistence
...
class AudioEngine {
  ...
  setVolume(v) {
    ...
    AppState.state.audioVolume = vol;
  ...
  startAmbient() {
    ...
    const targetVol = (AppState.state.audioVolume !== undefined ? AppState.state.audioVolume : 0.7) * 0.18;
    ...
    AppState.state.audioEnabled = true;
    AppState.saveAudio();
  }
  ...
  stopAmbient() {
    ...
    AppState.state.audioEnabled = false;
    AppState.saveAudio();
  }

  toggle() {
    if (this.isMuted) { this.startAmbient(); return true; }
    else { this.stopAmbient(); return false; }
  }

  // Section-specific filter modulation (changes "feel" per section)
  onSectionChange(idx) {
    ...
    // Short sting sound on section entry
  ...
  startDragTone() {
    ...
    const filter = this.ctx.createBiquadFilter();
    filter.type = 'lowpass'; filter.frequency.setValueAtTime(140, this.ctx.currentTime);
    this.dragOsc.connect(this.dragGain); this.dragGain.connect(filter); filter.connect(this.masterGain);
    this.dragOsc.start();
  }

  updateDragTone(speed) {
    if (this.isMuted || !this.initialized || !this.dragOsc) return;
    this.dragOsc.frequency.setTargetAtTime(60 + Math.min(speed * 380, 240), this.ctx.currentTime, 0.05);
    this.dragGain.gain.setTargetAtTime(0.02 + Math.min(speed * 0.35, 0.12), this.ctx.currentTime, 0.05);
  }

  stopDragTone() {
    if (!this.initialized || !this.dragOsc) return;
    const osc = this.dragOsc; const gain = this.dragGain;
    this.dragOsc = null; this.dragGain = null;
    if (gain) {
...
// Wire up bus events
bus.on('audio:click',       () => audioEngine.playClick());
bus.on('audio:hover',       () => audioEngine.playHover());
bus.on('audio:achievement', () => audioEngine.playAchievement());
bus.on('audio:toggle',      () => audioEngine.toggle());
bus.on('section:change',    idx => audioEngine.onSectionChange(idx));
